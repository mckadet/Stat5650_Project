...
class.sum <- function(truth, predicted, cap = "Classification Performance"){
  ...
  tab = data.frame(Metric = c("Percent Correctly Classified","Specificity","Sensitivity","Kappa","AUC"),
                   Percent = c(pcc,spec,sens,kap,au)
                  )
  return(tab %>% 
         kbl(caption = cap) %>% 
