...
# Modeling
After identifying what features would be most helpful for predicting heart disease or attack and handling the class imablance in our response, we used a variety of modeling strategies for prediction. In general, we performed a 70% split in our training and test data and evaluated the model performance based on the test data (after cross-validation).
...
class.sum(test_sample.lasso$HeartDiseaseorAttack, 
          predict(health.gbm, newdata = test_sample.lasso, type = "response", n.trees = 50), 
          cap = "Test Data ADA Performance")
```
...
results <- data.frame(PCC = c(76.99, 75.23, 77.09, 73.15, 71.40, 72.16, 77.10,
                      ...
                      Specificity = c(79.91, 79.07, 80.01, 88.25, 89.99, 89.76,
                                      79.53, 69.69,74.45,72.77,73.72,70.03,75.43),
                      Sensitivity = c(NA,NA,NA,NA,NA,NA,NA,82.00,87.36,
...
# Conclusion
There were many modeling strategies that proved effective in predicting heart disease or attack. Using random forest and LASSO, we identified features that were considered most important for prediction. Next, we handled the class imablance in the response using upsampling. After taking these measures and performning necessary transformations on the predictor variables, there were several models that performed with about 80\% overall classification accuracy as well as 79\% sensitivity and 75\% specificity. 
